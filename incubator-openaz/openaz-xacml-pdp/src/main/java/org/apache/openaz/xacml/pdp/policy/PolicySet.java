...
public class PolicySet extends PolicyDef {
    private final TargetedCombinerParameterMap<Identifier, PolicySetChild> policyCombinerParameters = new TargetedCombinerParameterMap<>();
    ...
    protected List<CombiningElement<PolicySetChild>> getCombiningPolicies() {
        if (this.combiningPolicies == null) {
            ...
            final Iterator<PolicySetChild> iterPolicies = this.getChildren();
            if (iterPolicies != null) {
                while (iterPolicies.hasNext()) {
                    final PolicySetChild policySetChild = iterPolicies.next();
                    this.combiningPolicies
                        .add(new CombiningElement<>(policySetChild,
    ...
    public PolicySet(final StatusCode statusCodeIn, final String statusMessageIn) {
        super(statusCodeIn, statusMessageIn);
    }

    public PolicySet(final StatusCode statusCodeIn) {
        super(statusCodeIn);
    }

    public PolicySet(final PolicySet policySetParent) {
     ...
     *            s.
     */
    public void setPolicyCombinerParameters(final Collection<TargetedCombinerParameter<Identifier, PolicySetChild>> policyCombinerParametersIn) {
        this.policyCombinerParameters.setCombinerParameters(policyCombinerParametersIn);
    }

    public void addPolicyCombinerParameter(final TargetedCombinerParameter<Identifier, PolicySetChild> policyCombinerParameter) {
        this.policyCombinerParameters.addCombinerParameter(policyCombinerParameter);
    }

    public void addPolicyCombinerParameters(final Collection<TargetedCombinerParameter<Identifier, PolicySetChild>> policyCombinerParametersIn) {
    ...
    public void setChildren(final Collection<PolicySetChild> policySetChildren) {
        this.children = null;
    ...
    public void addChild(final PolicySetChild policySetChild) {
        this.ensureChildren();
        this.children.add(policySetChild);
    }

    public void addChildren(final Collection<PolicySetChild> policySetChildren) {
        this.ensureChildren();
        this.children.addAll(policySetChildren);
    }

    public void replaceReference(final PolicyIdReferenceBase<PolicyDef> ref, final PolicyDef def){
      children.set(children.indexOf(ref), def);
    }
    ...
    public void setPolicyCombiningAlgorithm(final CombiningAlgorithm<PolicySetChild> combiningAlgorithmIn) {
        this.combiningAlgorithm = combiningAlgorithmIn;
    }

    @Override
    public EvaluationResult evaluate(final EvaluationContext evaluationContext) throws EvaluationException {
        /*
         ...
         * See if we match
         */
        final MatchResult thisMatchResult = this.match(evaluationContext);
         ...
         */
        final List<CombiningElement<PolicySetChild>> listCombiningElements = this.getCombiningPolicies();
        ...
        final EvaluationResult evaluationResultCombined = this.getPolicyCombiningAlgorithm()
