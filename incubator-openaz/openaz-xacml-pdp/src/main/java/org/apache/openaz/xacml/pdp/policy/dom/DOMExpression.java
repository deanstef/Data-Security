...
import org.apache.openaz.xacml.pdp.policy.expressions.SunfishObligationExpression;
...
/**
 * DOMExpression extends {@link org.apache.openaz.xacml.pdp.policy.Expression}
 * with methods for creation
...
public abstract class DOMExpression extends Expression {
  ...
  public static boolean isExpression(final Node nodeExpression) {
    final String nodeName = nodeExpression.getLocalName();
    return XACML3.ELEMENT_APPLY.equals(nodeName) || XACML3.ELEMENT_ATTRIBUTEDESIGNATOR.equals(nodeName)
        || XACML3.ELEMENT_ATTRIBUTESELECTOR.equals(nodeName) || XACML3.ELEMENT_ATTRIBUTEVALUE.equals(nodeName)
        || XACML3.ELEMENT_FUNCTION.equals(nodeName) || XACML3.ELEMENT_VARIABLEREFERENCE.equals(nodeName);
  }

  /**
   * Creates a new <code>Expression</code> of the appropriate sub-type based on
   * the name of the given
   * <code>Node</code>.
   *
   * @param nodeExpression
   *          the <code>Node</code> to parse
   * @param policy
   *          the {@link org.apache.openaz.xacml.pdp.policy.Policy} containing
   ...
   */
  public static Expression newInstance(final Node nodeExpression, final Policy policy)
      throws DOMStructureException {
    final Element elementExpression = DOMUtil.getElement(nodeExpression);
    final boolean bLenient = DOMProperties.isLenient();

    final String localName = elementExpression.getLocalName();
    if (DOMUtil.isInNamespace(elementExpression, XACML3.XMLNS)) {
      if (localName.equals(XACML3.ELEMENT_APPLY)) {
        return DOMApply.newInstance(elementExpression, policy);
      } else if (localName.equals(XACML3.ELEMENT_ATTRIBUTEDESIGNATOR)) {
        return DOMAttributeDesignator.newInstance(elementExpression);
      } else if (localName.equals(XACML3.ELEMENT_ATTRIBUTESELECTOR)) {
        return DOMAttributeSelector.newInstance(elementExpression);
      } else if (localName.equals(XACML3.ELEMENT_ATTRIBUTEVALUE)) {
      ...
      } else if (localName.equals(XACML3.ELEMENT_FUNCTION)) {
        return new Function(DOMUtil.getIdentifierAttribute(elementExpression, XACML3.ATTRIBUTE_FUNCTIONID));
      } else if (localName.equals(XACML3.ELEMENT_VARIABLEREFERENCE)) {
    ...
    } else if (DOMUtil.isInNamespace(elementExpression, "urn:sunfish")) {
      if (SUNFISH.SUNFISH_OBLIGATION_EXPRESSION.equals(localName)) {
        final SunfishObligationExpression expr = new SunfishObligationExpression();
        expr.setObligationId(elementExpression.getAttribute("ObligationId"));

        int length = elementExpression.getAttributes().getLength();

        for(int j=0; j<length; j++){
          Node node = elementExpression.getAttributes().item(j);
          expr.getOtherAttributes().put(new QName(node.getLocalName()), node.getNodeValue());
        }
        return expr;
      } else if (!bLenient) {
        throw DOMUtil.newUnexpectedElementException(nodeExpression);
  ...
  public static boolean repair(final Node nodeExpression) throws DOMStructureException {
    final Element elementExpression = DOMUtil.getElement(nodeExpression);
    if (DOMUtil.isInNamespace(elementExpression, XACML3.XMLNS)) {
