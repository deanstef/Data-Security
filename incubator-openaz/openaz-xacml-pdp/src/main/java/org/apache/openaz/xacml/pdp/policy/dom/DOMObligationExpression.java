...
/**
 * DOMObligationExpression extends
 * {@link org.apache.openaz.xacml.pdp.policy.ObligationExpression} with
...
public class DOMObligationExpression extends ObligationExpression {
   ...
   * <code>Node</code> representing a
   * XACML ObligationExpression element.
   *
   * @param nodeObligationExpression
   *          the <code>Node</code> representing the XACML ObligationExpression
   *          element
   * @param policy
   *          the {@link org.apache.openaz.xacml.pdp.policy.Policy} encompassing
   *          the
   *          ObligationExpression element
   * @return a new <code>ObligationExpression</code> parsed from the given
   *         <code>Node</code>
   * @throws DOMStructureException
   *           if there is an error parsing the <code>Node</code>
   */
  public static ObligationExpression newInstance(final Node nodeObligationExpression, final Policy policy)
      throws DOMStructureException {
    final Element elementObligationExpression = DOMUtil.getElement(nodeObligationExpression);
    final boolean bLenient = DOMProperties.isLenient();

    final DOMObligationExpression domObligationExpression = new DOMObligationExpression();

    try {
      final NodeList children = elementObligationExpression.getChildNodes();
      int numChildren;
        ...
        for (int i = 0; i < numChildren; i++) {
          ...
          if (DOMUtil.isElement(child)) {
            if ((DOMUtil.isInNamespace(child, XACML3.XMLNS)
                ...
                || (DOMUtil.isInNamespace(child, "urn:sunfish")
                    && "SunfishObligationExpression".equals(child.getLocalName()))) {
      ...
      final String string = DOMUtil.getStringAttribute(elementObligationExpression, XACML3.ATTRIBUTE_FULFILLON,
          !bLenient);
      final RuleEffect ruleEffectType = RuleEffect.getRuleEffect(string);
  ...
  public static boolean repair(final Node nodeObligationExpression) throws DOMStructureException {
    final Element elementObligationExpression = DOMUtil.getElement(nodeObligationExpression);
   ...
   *
   * @param nodeObligationExpressions
   *          the <code>Node</code> representing the XACML ObligationExpressions
   *          element
   * @param policy
   *          the <code>Policy</code> encompassing the ObligationExpressions
   *          element
   * @return a new <code>List</code> of <code>ObligationExpression</code>s
   *         parsed from the given
   *         <code>Node</code>
   * @throws DOMStructureException
   *           if there is an error parsing the <code>Node</code>
   */
  public static List<ObligationExpression> newList(final Node nodeObligationExpressions, final Policy policy)
      throws DOMStructureException {
    final Element elementObligationExpressions = DOMUtil.getElement(nodeObligationExpressions);
    final boolean bLenient = DOMProperties.isLenient();

    final List<ObligationExpression> listObligationExpressions = new ArrayList<>();

    final NodeList children = elementObligationExpressions.getChildNodes();
    int numChildren;
    if (children != null && (numChildren = children.getLength()) > 0) {
      for (int i = 0; i < numChildren; i++) {
        final Node child = children.item(i);
        if (DOMUtil.isElement(child)) {
          ...
          if (DOMUtil.isInNamespace(child, XACML3.XMLNS)) {
            if (XACML3.ELEMENT_OBLIGATIONEXPRESSION.equals(localName)) {
  ...
  public static boolean repairList(final Node nodeObligationExpressions) throws DOMStructureException {
    final Element elementObligationExpressions = DOMUtil.getElement(nodeObligationExpressions);
    boolean result = false;
