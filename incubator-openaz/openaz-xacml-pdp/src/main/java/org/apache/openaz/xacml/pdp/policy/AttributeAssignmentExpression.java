...
public class AttributeAssignmentExpression extends PolicyComponent {
    ...
    public AttributeAssignmentExpression(final StatusCode statusCodeIn, final String statusMessageIn) {
        super(statusCodeIn, statusMessageIn);
    }

    public AttributeAssignmentExpression(final StatusCode statusCodeIn) {
        super(statusCodeIn);
    }
    ...
    public AttributeAssignmentExpression(final Identifier categoryIn, final Identifier attributeIdIn, final String issuerIn,
    ...
    public void setCategory(final Identifier identifier) {
    ...
    public void setAttributeId(final Identifier identifier) {
    ...
    public void setIssuer(final String string) {
        this.issuer = string;
    }
    ...
    public AttributeAssignmentResult evaluate(final EvaluationContext evaluationContext,
        ...
        final Expression thisExpression = this.getExpression();
        if (thisExpression == null) {
            return AAR_NULL_EXPRESSION;
        }

        final ExpressionResult thisExpressionResult = thisExpression.evaluate(evaluationContext, policyDefaults);
        if (thisExpressionResult == null) {
            ...
            final List<AttributeAssignment> listAttributeAssignments = new ArrayList<>();
            if (thisExpressionResult.isBag()) {
                final Bag bagValues = thisExpressionResult.getBag();
                if (bagValues == null || bagValues.size() == 0) {
                    ...
                    final Iterator<AttributeValue<?>> iterBagValues = bagValues.getAttributeValues();
                    while (iterBagValues.hasNext()) {
                        final AttributeValue<?> attributeValue = iterBagValues.next();
                        //FIXME
                        StdMutableAttributeAssignment e = new StdMutableAttributeAssignment(attributeValue.getXPathCategory(),
                                                                                       this.getAttributeId(),
                                                                                       this.getIssuer(),
                                                                                       attributeValue);
                        listAttributeAssignments.add(e);
                    }
                }
            } else {
                listAttributeAssignments.add(new StdMutableAttributeAssignment(this.getCategory(), this
