...
public class DOMRule extends Rule {
     ...
     */
    public static Rule newInstance(final Node nodeRule, final Policy policy) throws DOMStructureException {
        final Element elementRule = DOMUtil.getElement(nodeRule);
        final boolean bLenient = DOMProperties.isLenient();

        final DOMRule domRule = new DOMRule();
        ...
        try {
            final NodeList children = elementRule.getChildNodes();
            int numChildren;
                      ...
                      final String childName = child.getLocalName();
                        if (DOMUtil.isInNamespace(child, XACML3.XMLNS)) {
                            if (XACML3.ELEMENT_DESCRIPTION.equals(childName)) {
                                ...
                                final Node nodeExpression = DOMUtil.getFirstChildElement(child);
            ...
            final String string = DOMUtil.getStringAttribute(elementRule, XACML3.ATTRIBUTE_EFFECT, !bLenient);
            final RuleEffect ruleEffect = RuleEffect.getRuleEffect(string);
    ...
    public static boolean repair(final Node nodeRule) throws DOMStructureException {
        final Element elementRule = DOMUtil.getElement(nodeRule);
        ...
        final String string = DOMUtil.getStringAttribute(elementRule, XACML3.ATTRIBUTE_EFFECT);
        final RuleEffect ruleEffect = RuleEffect.getRuleEffect(string);
        if (ruleEffect == null) {
