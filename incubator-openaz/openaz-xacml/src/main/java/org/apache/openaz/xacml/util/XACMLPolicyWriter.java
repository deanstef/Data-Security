...
public class XACMLPolicyWriter {
    ...
    private static JAXBContext policyTypeContext = initContextPolicyType();
    private static JAXBContext policySetTypeContext = initContextPolicySetType();

    private static JAXBContext initContextPolicyType(){

        try {
            return JAXBContext.newInstance(PolicyType.class);
        } catch (JAXBException e) {
            e.printStackTrace();
            //TODO do something more usefiö
        }

        return null;
    }

    private static JAXBContext initContextPolicySetType(){

        try {
            return JAXBContext.newInstance(PolicySetType.class);
        } catch (JAXBException e) {
            e.printStackTrace();
            //TODO do something more usefiö
        }
    ...
    public static Path writePolicyFile(Path filename, PolicySetType policySet) {
        ...
        try {
            ...
            Marshaller m = policySetTypeContext.createMarshaller();
    ...
    public static Path writePolicyFile(Path filename, PolicyType policy) {
        ...
        try {
            Marshaller m = policyTypeContext.createMarshaller();
            m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);
